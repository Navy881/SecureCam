...
class Camera(object):
    ...
    def motion_detect(self, running, video_file, show_edges,
        ...
        while(running):
            ...
            # Бинаризация разницы
            thresh = cv2.threshold(frameDelta, threshold_low, 255, cv2.THRESH_BINARY)[1]
            # Подамвления шума
            ...
            cv2.putText(frame, datetime.now().strftime("%d-%m-%Y %H:%M:%S%p"), (10, frame.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 0), 1)
            ...
            cv2.putText(frame1, datetime.now().strftime("%d-%m-%Y %H:%M:%S%p"), (10, frame1.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 0), 1)
    ...
    def real_time_detection(self, frame, net, CLASSES, COLORS, given_confidence):
